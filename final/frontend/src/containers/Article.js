...
import {DELETE_ARTICLE_MUTATION, GET_ARTICLE_QUERY, GET_ARTICLES_QUERY} from "../graphql";
...
const Article = () => {
    ...
    return (
        <Box sx={{
            marginTop: data?.getArticle.imageURL === '' ? '60px' : '55px',
            display: 'flex',
            width: '1100px',
            gap: '30px',
            marginX: 'auto'
            ...
            <Box sx={{width: '75%', background: 'white', borderRadius: '5px', margin: 'auto'}}>
                <Box sx={{width: '100%'}}>
                    {data?.getArticle.imageURL !== '' &&
                        <CardMedia component="img" height={500} image={data?.getArticle.imageURL}/>}
                </Box>
                <Typography variant='h4'
                            sx={{fontFamily: ['-apple-systems', 'sans-serif'], fontWeight: 700, margin: '10px 20px'}}>
            ...
            <Box sx={{width: '25%', background: 'white', height: 'fit-content', margin: '60px auto'}}>
                <Typography sx={{
                    margin: '10px', fontFamily: ['-apple-system', 'sans-serif'].join(','),
                    fontWeight: 700,
                }}>
                    Read More
                </Typography>
                {data2?.getArticles.filter(e => e.title !== data?.getArticle.title).map(
                    (e) => {
                        return (
                            <Box sx={{
                                margin: '5px 8px', fontSize: '14px', color: '#3a3a3a',
