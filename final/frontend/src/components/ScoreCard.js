        ...
        <Card className='card' sx={{borderRadius: '5px', boxShadow: '0px 0px 2px 0px rgba(0, 0, 0, 0.5)'}}>
            <CardContent sx={{height: '100%', alignItems: 'center'}}>
                <Grid className='grid' container spacing={2}>
                    ...
                    <Grid item xs={(game.period === 0)? 4:2} sx={{textAlign: 'center'}}><Typography variant="h6" sx={{
                ...
                {game.period !== 0 && <Grid container spacing={2} sx={{position: 'relative', top: '-5px'}}>
                    <Grid item><Button variant={'outlined'} sx={Gridstyles} onClick={() => {
                        navigate('/game/' + game.id)
                        setActive2('Box Score')
                    }}>Box Score</Button></Grid>
                    <Grid item><Button variant={'outlined'} sx={Gridstyles} onClick={() => {
                        navigate('/game/' + game.id)
                        setActive2('Team Stats')
                    }}>Team Stats</Button></Grid>
                    {game.status === 'Final' && <Grid item><Button variant='outlined' sx={Gridstyles}
                                                                   onClick={() => window.open(`https://www.youtube.com/results?search_query=${game?.visitor_team.name}+vs+${game?.home_team.name}`)}>Highlights</Button></Grid>}
                </Grid>}
            </CardContent>
        </Card>
    )
