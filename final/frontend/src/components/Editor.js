...
const Editor = () => {
    const {
        open, setOpen, publishArticle, content, title, description, image, headline, setContent, edit,
        ...
        <Modal open={open} onClose={() => setOpen(false)} sx={{overflowY: 'scroll'}}>
            <Container sx={{background: 'white', borderRadius: '15px', width: '900px', margin: '60px auto'}}>
                <Typography variant='h4' sx={{
                    position: 'relative',
                    top: '20px',
                    fontFamily: ['-apple-systems', 'sans-serif'],
                    fontWeight: 700
                }}>{edit ? 'Edit Article' : 'Write an Article'}</Typography>
                <Box sx={{
                    margin: '70px 0',
                    display: 'flex',
                    ...
                    <TextField error={error.title} required variant='filled' label="Title" size="small" value={title}
                               onChange={(e) => setTitle(e.target.value)}/>
                    <TextField error={error.description} required variant='filled' label="Description" size="small"
                               multiline value={description} onChange={(e) => setDescription(e.target.value)}/>
                    <TextField variant='filled' label="Image URL" size="small" multiline value={image}
