...
const NavBar = ({pages}) => {
    const {active, setActive, open, setOpen, setEdit, setContent, setDescription, setHeadline, setImage, setTitle} = useData()
    ...
    if (window.location.href.slice(-5) !== 'login') return (
        <Box className='Nav'>
            ...
            <Box>
                {isAuthenticated ?
                    <Box sx={{marginTop: '5px', position: 'relative', right: open &&'15px', display:'flex', gap:'5px'}}>
                        <Typography variant='h˙' sx={{fontWeight:'500', position:'relative', top:'5px', right:'5px'}}>
                            {user.nickname}
                        </Typography>
